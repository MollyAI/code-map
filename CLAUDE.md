...
- **Regression gate (INV-1 / INV-U1).** `code-map invariants --data <map>` exits non-zero on any violation — **INV-1** (within each layer, every rendered/core node's `display_name||name` is unique) and **INV-U1** (every node box fits its full label; no clip/ellipsis). Pure logic in `viewer/src/data/invariants.js` (reuses `layout/metrics.js`); exercised by `tests/invariants*.test.mjs` + `viewer/src/test/invariants.test.js`, and run on real fixtures by `eval/run.py invariants`. There is no `.github/workflows` — this gate + `node --test` are the enforcement path.
