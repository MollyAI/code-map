...
# Tune core selection (default 0.30/layer, cap 40/layer, floor 4/layer — the
# viewer renders core decls only, so the floor keeps thin layers from showing
# a single lonely box)
bin/code-map analyze --root . --out .code-map/raw_structure.json --core-percentile 0.15 --core-max-per-layer 60 --core-min-per-layer 2
