...
- **`architecture.yml`** —— AI 提议的架构，在第 2 阶段自动写入（依据完整提取结果决定），每次构建都会重新生成。可查看它以了解地图所用的分层结构。
...
模板自动检测覆盖上述 13 种形态，包括 **C / RTOS 内核**（可识别 `kernel`/`arch`/`drivers`/`Kconfig`/`BUILD.gn` 等信号）——其评分仅供参考，最终架构由第 2 阶段对照真实代码决定。
...
1. **提取**（Node + web-tree-sitter / WASM）——遍历项目，用对应语言的语法解析每个文件，构建依赖图、计算重要度。与架构无关、确定性强：只产出原始结构，尚不分层。可审计。
2. **决策与精炼**（Claude）——阅读完整提取结果（包结构、重要度、依赖图），手握真实代码**一次性**确定架构，再跑一遍确定性分层来划分层级、勾勒流程。为每个声明撰写一句话说明，修正分层，并补全解析器遗漏的内容。仅在 AI 判断真正有用之处消耗 token。
